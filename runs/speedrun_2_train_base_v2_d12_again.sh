...
SNAPSHOT_STEPS=1000  # 何ステップごとにモデルのスナップショットを保存するか

# d24 model (slightly undertrained to beat GPT-2 => decrease data:params ratio from compute optimal 10.5 (default) to 8)
torchrun --standalone --nproc_per_node=1 -m scripts.base_train -- --depth=$DEPTH --target-param-data-ratio=8 --device-batch-size=16 --fp8 --run=$WANDB_RUN --save-every $SNAPSHOT_STEPS
